...
def executor(tmp_project, phase_dir):
    ...
    with patch.object(ex, "HARNESS_ROOT", tmp_project):
        inst = ex.StepExecutor("0-mvp")
    # 내부 경로를 tmp_project 기준으로 재설정
    inst._harness_root = tmp_project
    inst._project_root = tmp_project
...
class TestBuildPreamble:
    ...
    def test_forbids_agent_commit(self, executor):
        result = executor._build_preamble("", "")
    ...
    def test_forbids_phase_status_changes(self, executor):
        result = executor._build_preamble("", "")
        assert "phase index의 status" in result
...
class TestInvokeClaude:
    def test_invokes_claude_with_correct_args(self, executor):
        ...
        assert any("git commit" in arg and "git push" in arg for arg in cmd)
        assert "PREAMBLE" in cmd[-1]
        assert "UI를 구현하세요" in cmd[-1]
        assert mock_run.call_args.kwargs["cwd"] == executor._project_root
    ...
    def test_saves_output_json(self, executor):
        ...
        with patch("subprocess.run", return_value=mock_result):
            output = executor._invoke_claude(step, "preamble")
        executor._record_attempt(step, 1, output, None, None)
        ...
        assert data["step"] == 2
        assert data["name"] == "ui"
        assert data["attempts"][0]["claude"]["exit_code"] == 0
...
# ---------------------------------------------------------------------------
# Harness-owned verification and state transitions
# ---------------------------------------------------------------------------

def claude_output(outcome="ready_for_verification", summary="done", blocked_reason=None):
    payload = {
        "outcome": outcome,
        "summary": summary,
    ...
    return {
        ...
        "stderr": "",
        "executed_at": "2026-01-01T00:00:00+0900",
    }


def verification(status="passed", failed_name="test"):
    exit_code = 0 if status == "passed" else 1
    return {
        "status": status,
        "verified_at": "2026-01-01T00:00:01+0900",
        "commands": [{
            "name": failed_name,
            "command": ["npm", "test"],
            "exit_code": exit_code,
            "stdout": "test stdout",
            "stderr": "test stderr" if exit_code else "",
            "executed_at": "2026-01-01T00:00:01+0900",
...
class TestVerificationRunner:
    def test_runs_all_commands_when_lint_fails(self, executor):
        verify = [
            {"name": "lint", "command": ["npm", "run", "lint"]},
            {"name": "build", "command": ["npm", "run", "build"]},
        ]
        results = [
            MagicMock(returncode=1, stdout="", stderr="lint failed"),
            MagicMock(returncode=0, stdout="built", stderr=""),
        ]
        with patch("subprocess.run", side_effect=results) as run:
            result = executor._run_verification(verify)

        assert run.call_count == 2
        assert result["status"] == "failed"
        assert [c["status"] for c in result["commands"]] == ["failed", "passed"]
        assert all(call.kwargs["cwd"] == executor._project_root for call in run.call_args_list)

    def test_timeout_is_recorded_as_failure(self, executor):
...
class TestHarnessOwnedCompletion:
    def _step(self, executor):
        return json.loads(executor._index_file.read_text())["steps"][2]
    ...
    def test_all_verify_commands_must_pass_before_completed(self, executor):
        ...
        assert step["verification"]["status"] == "passed"
        executor._commit_step.assert_called_once_with(2, "ui")

    def test_failure_log_is_passed_to_next_retry(self, executor):
        self._prepare(executor)
        prompts = []

        def invoke(_step, preamble):
            prompts.append(preamble)
            return claude_output()

        executor._invoke_claude = invoke
        executor._run_verification = MagicMock(side_effect=[
            verification("failed", "lint"),
            verification("passed"),
        ])
        ...
        assert "Harness 독립 검증에 실패했습니다" in prompts[1]
        assert "lint" in prompts[1]
        assert "test stderr" in prompts[1]

    def test_three_failures_mark_error_without_commit(self, executor):
        self._prepare(executor)
        executor._invoke_claude = MagicMock(return_value=claude_output())
        executor._run_verification = MagicMock(return_value=verification("failed"))

        with pytest.raises(SystemExit) as exc_info:
            executor._execute_single_step(self._step(executor), "guardrails")

        assert exc_info.value.code == 1
        assert executor._invoke_claude.call_count == 3
        assert self._step(executor)["status"] == "error"
        executor._commit_step.assert_not_called()

    def test_blocked_skips_verification(self, executor):
        self._prepare(executor)
        executor._invoke_claude = MagicMock(return_value=claude_output(
            outcome="blocked", summary="waiting", blocked_reason="API key required"
        ))
        executor._run_verification = MagicMock()

        with pytest.raises(SystemExit) as exc_info:
            executor._execute_single_step(self._step(executor), "guardrails")

        assert exc_info.value.code == 2
        assert self._step(executor)["status"] == "blocked"
        executor._run_verification.assert_not_called()
        executor._commit_step.assert_not_called()

    def test_claude_completed_write_is_overridden_when_verify_fails(self, executor):
        self._prepare(executor)
    ...
    def test_verification_uses_snapshot_when_claude_changes_index(self, executor):
        self._prepare(executor)
        original_verify = copy.deepcopy(self._step(executor)["verify"])

        def invoke(_step, _preamble):
            index = json.loads(executor._index_file.read_text())
            index["steps"][2]["verify"] = [
                {"name": "bypass", "command": ["true"]}
            ]
            executor._index_file.write_text(json.dumps(index))
            return claude_output()
        ...
        executor._run_verification = verify
        executor._execute_single_step(self._step(executor), "guardrails")

        assert seen == [original_verify]
        assert self._step(executor)["verify"] == original_verify

    def test_head_change_blocks_without_verification(self, executor):
        executor._git_head = MagicMock(side_effect=["before", "after"])
        executor._invoke_claude = MagicMock(return_value=claude_output())
        executor._run_verification = MagicMock()
        executor._commit_step = MagicMock(return_value=True)
...
class TestMainCli:
    ...
    def test_invalid_phase_dir_exits(self):
        with patch("sys.argv", ["execute.py", "nonexistent"]):
            with patch.object(ex, "HARNESS_ROOT", Path("/tmp/fake_nonexistent")):
                with pytest.raises(SystemExit) as exc_info:
                    ex.main()
                assert exc_info.value.code == 1
...
class TestCheckBlockers:
    def _make_executor_with_steps(self, tmp_project, steps):
        ...
        index = {"project": "T", "phase": "test", "project_root": ".", "steps": steps}
